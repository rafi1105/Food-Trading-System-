...
if(isset($_POST['update_product'])){
   ...
   $fetch_price = $conn->prepare("SELECT price FROM `products` WHERE id = ?");
   $fetch_price->execute([$pid]);
   $current_price = $fetch_price->fetchColumn();

   // Insert the current price into the price history table
   $insert_price_history = $conn->prepare("INSERT INTO `product_price_history` (product_id, price) VALUES (?, ?)");
   $insert_price_history->execute([$pid, $current_price]);
